...
graph1 <- ggplot(data = df_cours, aes(x = Quand, y = Nbre, group = 1)) +
  geom_point() +
  geom_line(color="steelblue") +
  theme(axis.text.x=element_text(angle=90, hjust=1)) +
  ylim(0, max(df_cours$Nbre) + 1) +
  xlab("Jour") 
...
#| title: Distribution de la fréquentation par cours

graph2 <- ggplot(data = df_cours, aes(x = Mois, y = Nbre)) +
  geom_boxplot(aes(fill=Jour)) +
  facet_grid(.~Jour)+
  stat_summary(fun=mean, geom="point", colour = "black", shape=10, size = 2) +
...
table_summary_cours <- df_cours %>%
    dplyr::summarise(
        obs=n(),
        min=round(min(Nbre, na.rm=T), 2),
        Q1=round(quantile(Nbre, 0.25, na.rm=T), 2),
        médiane=round(median(Nbre, na.rm=T), 2),
        moyenne=round(mean(Nbre, na.rm=T), 2),
        Q3=round(quantile(Nbre, 0.75, na.rm=T), 2),
        max=round(max(Nbre, na.rm=T), 2))
...
```{r}
#| label: frequentation-cours-jour
